...
/* update 2: life pattern + available study time - stable click-only */
(function(){
  state.lifeBlocks ||= [];
  save();
  ...
  function lifeAdviceText(){
    ...
    if(todaySleep && todaySleep<=270){
      return "오늘 수면 시간이 짧아. 핵심 과목 위주로 줄이는 게 좋아 보여.";
    }
    ...
    for(let h=0; h<24; h++){
      ...
      for(let d=1; d<=7; d++){
        ...
        const chips=blocksForDay(day).filter(b=>{
          const s=toMin(b.start), e0=toMin(b.end), e=e0<=s?e0+1440:e0;
          const cur=h*60;
          const cur2=cur+59;
          if(e0<=s){
            return (cur2>=s && cur<1440) || (cur2>=0 && cur<e0);
          }
          return cur2>=s && cur<e;
        }).map(b=>`<button type="button" class="life-chip ${b.type}" data-life-id="${b.id}">${esc(blockTitle(b))}</button>`).join("");
  ...
  function renderLifeList(){
    const box=q("#lifeBlockList");
    const section=q("#lifeBlockListSection");
    ...
    box.innerHTML=sorted.map(b=>`
      <div class="row">
        <div class="row-art"></div>
        <div>
          <div class="row-title">${dayLabels[b.day]} · ${esc(blockTitle(b))}</div>
          <div class="row-sub">${esc(b.start)}~${esc(b.end)} · ${typeLabels[b.type]||b.type}${b.repeatWeekly?" · 매주 반복":""}</div>
        </div>
        <div class="row-right">${typeLabels[b.type]||b.type}</div>
        <button type="button" class="more life-edit-btn" data-life-id="${b.id}">수정</button>
      </div>
  ...
  window.openLifeBlock=function(id=null){
    ...
    const modal=q("#lifeBlockModal");
    if(!modal) return alert("생활패턴 창을 찾지 못했어.");
    q("#lifeBlockModalTitle").textContent=b?"시간 블록 수정":"시간 블록 추가";
    q("#lifeBlockId").value=b?.id||"";
    q("#lifeDay").value=String(b?.day ?? todayDow());
    q("#lifeType").value=b?.type||"school";
    q("#lifeName").value=b?.name||"";
    q("#lifeStart").value=b?.start||"08:00";
    q("#lifeEnd").value=b?.end||"16:00";
    const repeat=q("#lifeRepeatWeek");
    if(repeat) repeat.checked=!!b?.repeatWeekly;
    const all=q("#lifeAllDays");
    if(all){ all.checked=false; all.closest("label").style.display=b?"none":""; }
    q("#deleteLifeBlockBtn")?.classList.toggle("hidden",!b);
    modal.classList.add("show");
  };

  function saveLifeBlock(){
    const id=q("#lifeBlockId")?.value || "";
    const block={
      day:Number(q("#lifeDay")?.value ?? todayDow()),
      type:q("#lifeType")?.value || "school",
      name:(q("#lifeName")?.value || "").trim(),
      start:q("#lifeStart")?.value || "",
      end:q("#lifeEnd")?.value || "",
      repeatWeekly:!!q("#lifeRepeatWeek")?.checked
    };
    if(!block.start || !block.end){ alert("시작/끝 시간을 입력해줘."); return; }
    if(block.start===block.end){ alert("시작 시간과 끝 시간이 같으면 안 돼."); return; }
    if(!Array.isArray(state.lifeBlocks)) state.lifeBlocks=[];

    if(id){
      const target=state.lifeBlocks.find(x=>x.id===id);
      if(target) Object.assign(target, block);
    }else{
      if(q("#lifeAllDays")?.checked){
        for(let d=0; d<7; d++){
    ...
    save();
    q("#lifeBlockModal")?.classList.remove("show");
    renderLife();
  ...
  document.addEventListener("click",function(e){
    const add=e.target.closest("#addLifeBlockBtn");
    if(add){ e.preventDefault(); e.stopPropagation(); window.openLifeBlock(); return; }

    const saveBtn=e.target.closest("#saveLifeBlockBtn");
    if(saveBtn){ e.preventDefault(); e.stopPropagation(); saveLifeBlock(); return; }

    const del=e.target.closest("#deleteLifeBlockBtn");
    if(del){ e.preventDefault(); e.stopPropagation(); deleteLifeBlock(); return; }

    const chip=e.target.closest(".life-chip,.life-edit-btn");
    if(chip?.dataset?.lifeId){ e.preventDefault(); e.stopPropagation(); window.openLifeBlock(chip.dataset.lifeId); return; }
  },true);

  const oldRender=window.render || (typeof render==="function" ? render : null);
  if(typeof oldRender==="function" && !window.__lifeRenderWrapped){
    window.__lifeRenderWrapped=true;
